...
package com.devourer.alexb.diaryforthecoolestboys.FingerprintLibrary;
...
public class FingerprintDialogCompatV23 extends DialogFragment {
    ...
    public void onViewCreated(@NonNull final View view, @Nullable final Bundle savedInstanceState) {
        ...
        mSymbolView = view.findViewById(R.id.fingerprint_symbol_iv);
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                mSymbolView.setState(SwirlView.State.ON);
            }
        }, 500 /* 1 seconds */);
        ...
        if (getArguments().containsKey(ARG_TITLE)) {
            final TextView titleTv = view.findViewById(R.id.title_tv);
        ...
        if (getArguments().containsKey(ARG_SUBTITLE)) {
            final TextView subtitleTv = view.findViewById(R.id.subtitle_tv);
        ...
        if (getArguments().containsKey(ARG_NEGATIVE_BUTTON_TITLE)) {
            ...
            final Button negativeButton = view.findViewById(R.id.negative_btn);
        ...
        try {
            ImageView appIconIv = view.findViewById(R.id.app_icon_iv);
        ...
        if (cryptoObject != null) {
            final FingerprintManager.AuthenticationCallback authCallback = new FingerprintManager.AuthenticationCallback() {
                ...
                public void onAuthenticationFailed() {
                    mSymbolView.setState(SwirlView.State.ERROR);
                    new Handler().postDelayed(new Runnable() {
                        @Override
                        public void run() {
                ...
                public void onAuthenticationSucceeded(FingerprintManager.AuthenticationResult result) {
                    mSymbolView.setState(SwirlView.State.OFF);
                    mSymbolView.setState(SwirlView.State.ON);
                    mStatusText.setTextColor(getResources().getColor(R.color.colorOkActionSnackbar));
                    mStatusText.setText("Authentication success!");
                    new Handler().postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            mStatusText.setText("");
                            mStatusText.setTextColor(getResources().getColor(R.color.colorLogOut));
                        }
                    }, 700 );
